...
class SubscriberService {
  async getActiveSubscribers(
    filters: { wantsDiscountEmails?: boolean } = {}
  ): Promise<any[]> {
    try {
...
async function getDiscountProducts(): Promise<any[]> {
  return [
    ...
    },
    {
      ...
      description:
        "Track your fitness and stay connected with our latest smart watch.",
    },
  ];
}
...
class EmailService {
  ...
  constructor() {
    ...
    if (
      ...
      throw new Error(
        "Missing EMAIL_HOST, EMAIL_USER or EMAIL_PASS in environment variables"
      );
    }
  ...
  async sendEmail(to: string, subject: string, html: string): Promise<boolean> {
    try {
      const info = await this.transporter.sendMail({
        from: `"The Akram Company" <${process.env.EMAIL_USER}>`,
        to,
        subject,
        html,
      });
      ...
      productsHTML += `
        <div style="margin-bottom: 20px; border-bottom: 1px solid #eee; padding-bottom: 15px;">
          ...
          <p style="margin: 5px 0; font-size: 16px;">
            <span style="text-decoration: line-through; color: #999;">$${product.originalPrice.toFixed(
              2
            )}</span>
            <span style="color: #3AA39F; font-weight: bold; margin-left: 10px;">$${product.discountPrice.toFixed(
              2
            )}</span>
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            ...
            }/products/discount" class="button">Shop All Discounts</a>
          ...
          <div class="footer">
            ...
            <p>To unsubscribe from discount notifications, <a href="${
              process.env.WEBSITE_URL || "#"
            }/unsubscribe?type=discount" style="color: #3AA39F;">click here</a></p>
          </div>
        </div>
      </body>
      </html>
  ...
  async sendDailyDiscountEmails(): Promise<void> {
    try {
      ...
      if (discountProducts.length === 0) {
        logger.info(
          "No discounted products available today, skipping discount email"
        );
        return;
      }

      // Send emails
      const subject = `Today's Special Discounts - Up to ${Math.round(
        (1 -
          discountProducts[0].discountPrice /
            discountProducts[0].originalPrice) *
          100
      )}% OFF!`;
      ...
      for (const subscriber of subscribers) {
        const html = this.generateDiscountEmailHTML(
      ...
      logger.info(
        `Daily discount emails sent: ${successCount}/${subscribers.length}`
      );
    } catch (error) {
...
export const sendManualDiscountEmail = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    if (
      ...
      res
        .status(400)
        .json({ success: false, message: "No subscribers specified" });
      return;
    }
    ...
    if (discountProducts.length === 0) {
      res
        .status(404)
        .json({ success: false, message: "No discount products available" });
      return;
    }

    // Send emails
    const subject = `Special Offer - Up to ${Math.round(
      (1 -
        discountProducts[0].discountPrice / discountProducts[0].originalPrice) *
        100
    )}% OFF!`;

    let successCount = 0;
    for (const subscriber of subscribers) {
      const html = emailService.generateDiscountEmailHTML(
        subscriber.firstName,
        discountProducts
      );
